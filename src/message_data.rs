...
impl MessageData {
    ...
    pub fn is_message_similar(&self, new_message: &str) -> bool {
        strsim::jaro(&self.message, new_message) > 0.85
    }

    pub fn reconstruct_message(&mut self, s2: &str) {
        let mut buf = String::with_capacity((self.message.len() + s2.len()) / 2 + 1);
        let mut i = self.message.chars();
        let mut j = s2.chars();
...
mod test {
    ...
    pub fn test_string_recreation() {
        ...
        for test_case in test_cases {
            ...
            assert_eq!(test_case.expected, message_data.message);
        }
    }
}